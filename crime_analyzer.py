...
class CrimeDataAnalyzer:
    ...
    def load_indianapolis_data(self, csv_path: str) -> pd.DataFrame:
        """
        Load and process Indianapolis crime CSV data with memory-efficient chunking.
        Uses chunked reading to avoid loading entire file into memory at once.
        """
        print("Loading Indianapolis crime data (memory-efficient mode)...")

        try:
            # Load only required columns for faster processing
            required_cols = ['LATITUDE', 'LONGITUDE', 'CRIME']

            # Use chunked reading to reduce memory footprint
            chunk_size = 10000  # Process 10k rows at a time
            valid_chunks = []
            total_rows = 0
            valid_rows = 0

            # Read and filter in chunks to minimize peak memory usage
            for chunk in pd.read_csv(csv_path, usecols=required_cols, chunksize=chunk_size, low_memory=True):
                total_rows += len(chunk)

                # Vectorized coordinate validation - much faster than iterative filtering
                lat_mask = ((chunk['LATITUDE'] >= GeoBoundaries.INDY_SOUTH) &
                            (chunk['LATITUDE'] <= GeoBoundaries.INDY_NORTH) &
                            (chunk['LATITUDE'] != 0))

                lon_mask = ((chunk['LONGITUDE'] >= GeoBoundaries.INDY_WEST) &
                            (chunk['LONGITUDE'] <= GeoBoundaries.INDY_EAST) &
                            (chunk['LONGITUDE'] != 0))

                # Combined mask for all valid coordinates
                valid_mask = lat_mask & lon_mask & chunk['LATITUDE'].notna() & chunk['LONGITUDE'].notna()
                filtered_chunk = chunk[valid_mask]

                if len(filtered_chunk) > 0:
                    # Keep only essential columns, use numpy arrays for efficiency
                    valid_chunks.append({
                        'latitude': filtered_chunk['LATITUDE'].values.copy(),
                        'longitude': filtered_chunk['LONGITUDE'].values.copy(),
                        'crime_type': filtered_chunk['CRIME'].values.copy()
                    })
            ...
            print(f"Filtered {valid_rows}/{total_rows} valid records ({valid_rows*100//total_rows if total_rows > 0 else 0}%)")

            # Combine chunks efficiently
            if valid_chunks:
                processed_df = pd.DataFrame({
                    'latitude': np.concatenate([c['latitude'] for c in valid_chunks]),
                    'longitude': np.concatenate([c['longitude'] for c in valid_chunks]),
                    'crime_type': np.concatenate([c['crime_type'] for c in valid_chunks]),
                    'city': 'Indianapolis'
                })
    ...
    def analyze_crime_data(self, indy_csv_path: str, wl_json_path: str) -> Dict:
        """
        Memory-optimized complete crime data analysis pipeline.
        Includes explicit garbage collection to minimize memory footprint.
        """
        print("Starting fast crime data analysis (memory-optimized)...")

        # Load datasets
        indy_df = self.load_indianapolis_data(indy_csv_path)
        gc.collect()  # Clean up after loading Indianapolis data

        wl_df = self.load_west_lafayette_data(wl_json_path)
        gc.collect()  # Clean up after loading West Lafayette data
        ...
        gc.collect()  # Clean up after density calculation

        # Calculate optimal multiplier
        multiplier = self.calculate_optimal_crime_weight_multiplier()

        # Compile results
        total_crimes = len(indy_df) + len(wl_df)

        results = {
            'total_crime_count': total_crimes,
            'indianapolis_count': len(indy_df),
            'west_lafayette_count': len(wl_df),
            'crime_types_mapped': len(crime_weights),
            'crime_type_weights': crime_weights,
            'crime_weight_multiplier': multiplier,
            'density_grid_shape': density_grid.shape if density_grid.size > 0 else (0, 0),
            'analysis_complete': True
        }

        print(f"Crime analysis complete: {total_crimes} total records analyzed")
        print(f"Memory cleanup: {gc.collect()} objects collected")
