...
Memory Optimization:
- Graphs share the same underlying structure (view-based when possible)
- Edge weights calculated on-demand or stored efficiently
...
class WeightedGraphBuilder:
    ...
    def load_base_graph(self, graph: nx.MultiDiGraph) -> bool:
        """
        Load the base road network graph (memory-optimized).
        ...
        # Use the graph directly to avoid unnecessary copy (memory optimization)
    ...
    def create_weighted_graphs(self) -> Dict[str, nx.MultiDiGraph]:
        ...
        Memory-optimized: creates graphs sequentially with cleanup between.
        ...
        print("Creating weighted graphs for all route types (memory-optimized)...")

        self.weighted_graphs = {}
        
        for idx, (route_name, route_type) in enumerate(self.route_types.items(), 1):
            print(f"\n[{idx}/4] Creating {route_name} graph...")
